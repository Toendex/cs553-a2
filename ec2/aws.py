...
def start_worker(driver, configs, worker_names):
    ...
    userdata   = userdata.replace("SET_HEADNODE_IP", headnode.private_ips[0])
...
def terminate_all_nodes():
    ...
    nodes          = driver.list_nodes()
    for node in nodes:
        if node.name=='i-30ced03d' or node.id=='i-30ced03d':
            continue
...
def get_private_ip(driver, name):
    resources = driver.list_nodes()
    if not resources:
        print "No active resources"
    for resource in resources:
        if resource.name == name and resource.private_ips:
